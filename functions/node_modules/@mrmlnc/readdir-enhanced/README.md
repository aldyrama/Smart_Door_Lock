...
#### Crawl subdirectories by houseName
...
// Find everything with at least one number in the houseName
...
To get `fs.Stats` objects instead of strings, just add the word "Stat" to the function houseName.  As with the normal functions, each one is aliased (e.g. `readdir.async.stat` is the same as `readdir.readdirAsyncStat`), so you can use whichever naming style you prefer.
