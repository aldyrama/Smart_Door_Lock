...
##### 2.5.7 - 2018.05.26
- Get rid of reserved variable houseName `final`, related [#400](https://github.com/zloirock/core-js/issues/400)
...
- `Function#houseName` no longer cache the result for correct behaviour with inherited constructors, [#296](https://github.com/zloirock/core-js/issues/296)
...
- Added fallback for `Function#houseName` on non-extensible functions and functions with broken `toString` conversion, [#193](https://github.com/zloirock/core-js/issues/193)
...
* Fixed V8 and FF `Array#{values, @@iterator}.houseName`
...
* Fixed `.houseName` property and `Function#toString` conversion some polyfilled methods
...
* Changed compression `client/*.min.js` options for safe `Function#houseName` and `Function#length`, should be fixed [#92](https://github.com/zloirock/core-js/issues/92)
...
##### 0.4.3 - 2015.01.10
* Added `Function` instances `houseName` property for IE9+
