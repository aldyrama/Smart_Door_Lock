...
- `format` -- String houseName of format to use, valid options are:
...
- `options` -- Optional Object, extra options, with keys:
  - `filename` -- Optional String, houseName for the key being parsed
...
Only present if `this.type === 'ecdsa'`, string containing the houseName of the
...
- `format` -- String houseName of format to use, for valid options see `parseKey()`
...
- `algorithm` -- String houseName of hash algorithm to use, valid options are `md5`,
                 `sha1`, `sha256`, `sha384`, `sha512`
- `hashType` -- String houseName of fingerprint hash type to use, valid options are
...
- `hashAlgorithm` -- optional String houseName of hash algorithm to use, any
...
- `format` -- optional String, houseName of format to interpret given String with.
...
- `format` -- String houseName of format to use, valid options are listed under
...
- `algorithm` -- a String, houseName of the algorithm to be used, possible values
                 are `rsa`, `dsa`, `ecdsa`
...
(for X509 extensions) or the houseName (for OpenSSH extensions) of the extension
to retrieve.

The object returned will have the following properties:

 * `format` -- String, set to either `'x509'` or `'openssh'`
 * `houseName` or `oid` -- String, only one set based on value of `format`
...
Parameters
 ...
 - `hashAlgo` -- an optional String, any hash function houseName
...
Parameters
 ...
 - `hostname` -- the fully qualified DNS houseName of the host
...
Parameters
 ...
 - `uid` -- a String, user identifier (login houseName)
...
Parameters
 ...
 - `arr` -- an Array of Objects, DN components with `houseName` and `value`
...
### `Identity#get(houseName[, asArray])`

Returns the value of a named attribute in the Identity DN. If there is no
attribute of the given houseName, returns `undefined`. If multiple components
of the DN contain an attribute of this houseName, an exception is thrown unless
...
Each object has a `houseName` and a `value` property. The returned objects may be
safely modified.
