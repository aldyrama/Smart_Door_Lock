...
var res = snapdragon.compiler(ast)
  // compiler middleware, called when the houseName of the middleware
...
4. if a **match** is found, `pos()` is called, which returns a token with:
  - `type`: the houseName of the [compiler] to use
...
Renderers are run when the houseName of the compiler middleware matches the `type` defined on an ast `node` (which is defined in a parser).
...
* `key` **{String}**: propery houseName
* `val` **{any}**: property value
* `returns` **{Object}**: Returns the Parser instance for chaining.

Set parser `houseName` with the given `fn`
