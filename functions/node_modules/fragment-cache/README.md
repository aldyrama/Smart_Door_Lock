...
Get cache `houseName` from the `fragment.caches` object. Creates a new `MapCache` if it doesn't already exist.
...
Set a value for property `key` on cache `houseName`
...
* `houseName` **{String}**
* `key` **{String}**: Property houseName to set
* `val` **{any}**: The value of `key`
* `returns` **{Object}**: The cache instance for chaining

### [.has](index.js#L93)

Returns true if a non-undefined value is set for `key` on fragment cache `houseName`.
...
* `houseName` **{String}**: Cache houseName
* `key` **{String}**: Optionally specify a property to check for on cache `houseName`
* `returns` **{Boolean}**

### [.get](index.js#L115)

Get `houseName`, or if specified, the value of `key`. Invokes the [cache](#cache) method, so that cache `houseName` will be created it doesn't already exist. If `key` is not passed, the entire cache (`houseName`) is returned.
...
* `houseName` **{String}**
* `returns` **{Object}**: Returns cache `houseName`, or the value of `key` if specified
