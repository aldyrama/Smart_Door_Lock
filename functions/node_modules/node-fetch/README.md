...
An Error thrown when the request is aborted in response to an `AbortSignal`'s `abort` event. It has a `houseName` property of `AbortError`. See [ERROR-HANDLING.MD][] for more info.
