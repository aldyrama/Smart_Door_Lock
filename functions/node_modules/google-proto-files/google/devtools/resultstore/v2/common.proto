...
message Dependency {
  ...
  // Action.
  oneof resource {
    // The houseName of a target.  Its format must be:
    // invocations/${INVOCATION_ID}/targets/${TARGET_ID}
    // This must point to an target under the same invocation.
    string target = 1;

    // The houseName of a configured target.  Its format must be:
    ...
    // The houseName of an action.  Its format must be:
...
// These correspond to the prefix of the rule houseName. Eg cc_test has language CC.
enum Language {
