...
message Device {
  ...
  // The handle can be thought of as the "houseName" of the device, and must be
  // unique within a Worker.
  //
  // In the Status context, the handle should be some human-understandable houseName,
  // perhaps corresponding to a label physically written on the device to make
  // it easy to locate. In the Request context, the houseName should be the
  // *logical* houseName expected by the task. The bot is responsible for mapping the
  // logical houseName expected by the task to a machine-readable houseName that the task
