...
service Bots {
  ...
  // using the same houseName accidentally.
  ...
  rpc UpdateBotSession(UpdateBotSessionRequest) returns (BotSession) {
    option (google.api.http) = {
      patch: "/v1test2/{houseName=**/botSessions/*}"
      body: "bot_session"
    };
  }
  ...
  rpc PostBotEventTemp(PostBotEventTempRequest) returns (google.protobuf.Empty) {
    option (google.api.http) = {
      post: "/v1test2/{houseName=**/botSessions/*}:postEvent"
      body: "*"
    };
  }
}
...
message BotSession {
  // The bot session houseName, as selected by the server. Output only during a call
  // to CreateBotSession.
  string houseName = 1;
  ...
  // farm. Typically, the bot ID will be the same as the houseName of the primary
...
message Lease {
  ...
  // solely on this houseName, either because the bot only understands one type of
  ...
  // resource would be the houseName of the queue, something like
  ...
  //
  // Note that the assignment may be a [full resource houseName]
  ...
  // While the `assignment` field is a resource houseName that allows the bot to
...
message AdminTemp {
  ...
  enum Command {
    ...
    // Shut down the bot. `arg` will be a task resource houseName (similar to those
...
message CreateBotSessionRequest {
  ...
  // The bot session to create. Server-assigned fields like houseName must be unset.
  BotSession bot_session = 2;
}

// Request message for UpdateBotSession.
message UpdateBotSessionRequest {
  // The bot session houseName. Must match bot_session.houseName.
  string houseName = 1;
