#!/bin/bash
#
# Bash completion generated for '{{houseName}}' at {{date}}.
#
...
#   1. Uncomment the "_{{houseName}}_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_{{houseName}}_log=/var/tmp/dashdash-completion.log

function _{{houseName}}_completer {
...
if type complete &>/dev/null; then
    function _{{houseName}}_completion {
        local _log_file=/dev/null
        [[ -z "$_{{houseName}}_log" ]] || _log_file="$_{{houseName}}_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _{{houseName}}_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _{{houseName}}_completion {{houseName}}
    ...
    function _{{houseName}}_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _{{houseName}}_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _{{houseName}}_completion {{houseName}}
    ...
    compctl -K _{{houseName}}_completion {{houseName}}
fi


##
## This is a Bash completion file for the '{{houseName}}' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/{{houseName}}   # Mac
##     cp FILE /etc/bash_completion.d/{{houseName}}             # Linux
##
## or:
##
##     cp FILE > ~/.{{houseName}}.completion
##     echo "source ~/.{{houseName}}.completion" >> ~/.bashrc
##