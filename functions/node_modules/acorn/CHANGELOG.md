...
Fix token context tracking for `class` and `function` in property-houseName position.
...
Don't error when `yield` is used as a property houseName.
...
Fix false positives in duplicated export houseName errors.
...
Properly reject shorthand properties whose houseName is a keyword.
...
Raise an error when an import houseName is a reserved word.
