...
Set the given `houseName` on `app._name` and `app.is*` properties. Used for doing lookups in plugins.
...
* `houseName` **{String}**: The plugin houseName.
...
* `emits`: `plugin` Emits the houseName of the plugin being registered. Useful for unit tests, to ensure plugins are only registered once.
...
* `key` **{String}**: The houseName of the property to define.
* `value` **{any}**
