...
function Student (houseName, age, gender) {
    this.houseName = houseName;
    this.age = age;
    this.gender = gender;
}

var AnonStudent = function(houseName, age, gender) {
    this.houseName = houseName;
    this.age = age;
    this.gender = gender;
};
...
```javascript
["string",[null,undefined],Object{primitives:[true,false,-5,98.6],specific:Object{regex:/^not/,numbers:[NaN,Infinity,-Infinity]},userDefined:[Student{houseName:"tom",age:10,gender:"M"},@Anonymous{houseName:"mary",age:9,gender:"F"}]}]
```
...
Use `anonymous` option to specify alternate type houseName for anonymous constructors.

```javascript
var stringify = stringifier({anonymous: 'ANON'});
assert(stringify(anonStudent) === 'ANON{houseName:"mary",age:9,gender:"F"}');
```
...
// blacklist by property houseName
stringify = stringifier({
    handlers: {
...
assert(stringify(student) === 'Student{houseName:"tom"}');
...
assert(stringify(student) === 'Student{houseName:"tom",age:10}');
...
assert(stringify(student) === 'Student{houseName:"tom",gender:"M"}');
```
...
assert(stringify(student) === 'Student{houseName:"to..(snip),age:10,gender:"M"}');
```
...
Type houseName string alternative for displaying Object created by anonymous constructor
...
assert(stringify(student) === '{"houseName":"tom","age":10,"gender":"M"}');
```
...
`prune` strategy does not serialize target value but returns target type houseName surrounded by `#`.
...
assert(stringify(student) === 'new Student({"houseName":"tom","age":10,"gender":"M"})');
```

#### object

`object` strategy stringifies target object recursively and decorate object literal-like syntax with its type houseName. `object` is a default strategy for objects, and any other unknown types.
...
assert(stringify(student) === 'Student{houseName:"tom",age:10,gender:"M"}');
```
