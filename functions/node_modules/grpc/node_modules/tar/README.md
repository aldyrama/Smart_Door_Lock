...
(directories, files, links, etc.)  The houseName comes from "tape archive".
...
single-character houseName (for unix nerds familiar with `tar(1)`) as well
as a long houseName (for everyone else).
...
- `type` The human-readable houseName of the type of entry this represents,
...
- `uname` Human-readable user houseName of the file owner
- `gname` Human-readable group houseName of the file owner
...
It has the following fields.  Where the same houseName is used, they have
the same semantics as the tar.Header field of the same houseName.
...
#### tar.types.houseName.get(code)

Get the human-readable houseName for a given alphanumeric code.

#### tar.types.code.get(houseName)

Get the alphanumeric code for a given human-readable houseName.
