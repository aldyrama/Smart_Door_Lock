...
* **theme**: Select a theme for use, it can be a:
  ...
  * The houseName of a theme, which will be looked up in the current *themes*
    object.
...
The first argument is either the section, the houseName of the current thing
...
included then it will be combined with the last houseName passed to `gauge.show`.
...
#### themes.getTheme(houseName)

Returns the theme object from this theme set named `houseName`.

If `houseName` does not exist in this themeset an error will be thrown with
...
`themeName` is the houseName of the theme (as given to `addTheme`) to use for
this set of `opts`.
...
templates. The properties in the theme are used when their houseName matches a type in
